...
def predecir_fourier_single_task(comb, df_ventas_grouped, df_restricciones):
    ...
    fechas_all = pd.date_range(start=sub_df["inicio_semana"].min(), periods=len(t_all), freq="W-MON")
    fechas_fut = pd.date_range(start=sub_df["inicio_semana"].max() + pd.Timedelta(weeks=1), periods=n_periods, freq="W-MON")
...
def main():
    ...
    ventas_futuras = df_ventas[mask_futuro].groupby(['CLIENTE_ID_KEY','PRODUCTO_ID_KEY']).size().reset_index(name='ventas_futuras')
    ventas_prob = ventas_prob.merge(ventas_futuras, on=['CLIENTE_ID_KEY','PRODUCTO_ID_KEY'], how='left')
    ...
    max_combinaciones = {"Chico":17,"Mediano":27,"Grande":36,"Extragrand":45}
    max_default = 9
    tamanos = df_ventas.groupby("CLIENTE_ID_KEY")["SEGMENTACION"].agg(lambda x: x.mode().iloc[0] if not x.mode().empty else "desconocido").to_dict()
    combinaciones_filtradas["SEGMENTACION"] = combinaciones_filtradas["CLIENTE_ID_KEY"].map(tamanos)
    combinaciones_filtradas["MAX_COMB"] = combinaciones_filtradas["SEGMENTACION"].str.lower().map(max_combinaciones).fillna(max_default).astype(int)
    combinaciones_filtradas.sort_values(["CLIENTE_ID_KEY","probabilidad_venta"], ascending=[True,False], inplace=True)
    combinaciones_filtradas = combinaciones_filtradas.groupby("CLIENTE_ID_KEY", group_keys=False).apply(lambda x: x.head(int(x["MAX_COMB"].iloc[0]))).reset_index(drop=True)
    ...
    resultados = []
    for comb in tqdm(combinaciones_filtradas['comb'].tolist(), desc="Prediciendo combinaciones", file=sys.stdout):
    ...
    if resultados:
        ...
        # ================================================
        # Ajuste promedio semanal por cliente
        # ================================================
        limites_segmentacion = {"Micro":74,"Chico":157,"Mediano":325,"Grande":513,"Extragrand":995}
        segmentaciones = df_ventas.groupby("CLIENTE_ID_KEY")["SEGMENTACION"].agg(lambda x: x.mode().iloc[0]).to_dict()
        df_result['SEGMENTACION'] = df_result['CLIENTE_ID_KEY'].map(segmentaciones)
        df_result['SEMANA'] = df_result['FECHA_PREDICCION'].dt.to_period('W').apply(lambda r: r.start_time)

        for cliente_id, grupo in df_result.groupby('CLIENTE_ID_KEY'):
            seg = grupo['SEGMENTACION'].iloc[0]
            limite = limites_segmentacion.get(seg,None)
            if limite is None: continue
            for semana, df_semana in grupo.groupby('SEMANA'):
                total_semana = df_semana['PREDICCION'].sum()
                if total_semana > limite:
                    factor = limite / total_semana
                    df_result.loc[df_semana.index,'PREDICCION'] *= factor

        df_result['PREDICCION'] = np.round(df_result['PREDICCION'])

        # ================================================
        # Ajuste global al 80% del real
        # ================================================
        ...
        print(f"Predicciones guardadas en {SALIDA_PREDICCIONES}")
    else:
